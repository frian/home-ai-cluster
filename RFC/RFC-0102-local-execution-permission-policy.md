...
deterministic order. If no such candidate remains to be considered, the request
fails immediately with a distinct HAC-owned execution-permission meaning.
...
Accepted RFC-0098 defines execution availability as distinct from static
eligibility, health, reachability, and fallback safety. Accepted RFC-0099 limits
the truth to HAC-owned permission to begin an independent execution, not
runtime capacity. Accepted RFC-0100 scopes the first proof to one ordinary
process-local adapter-dispatch boundary, and Accepted RFC-0101 represents its
...
If the considered caller-local candidate is not permitted, HAC continues
considering the next statically eligible, not-yet-contacted candidate in the
existing deterministic order. The skipped local candidate must not enter its
adapter invocation, contact its runtime, count as a failed execution attempt,
or trigger RFC-0028 fallback.
...
## Relationship to RFC-0098 through RFC-0101

This RFC consumes Accepted RFC-0098's execution-availability semantics,
Accepted RFC-0099's HAC authority boundary, Accepted RFC-0100's first
process-local scope, and Accepted RFC-0101's active execution-interval
cardinality.

If any prerequisite is later superseded or materially changed, this RFC must be
reviewed again. The bounded representation implementation merged through PR
#654 demonstrates that RFC-0101's representation is implementable, but that
implementation remains evidence rather than an architectural prerequisite for
...
Rejected when a statically eligible alternative exists, because the bounded
goal is to share independent work across existing explicit nodes. With
`local_only` or no alternative, immediate failure remains the decision.
...
pre-transmission remote execution-availability fact. The asymmetry is
intentional.
