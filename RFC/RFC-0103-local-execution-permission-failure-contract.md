...
When Draft RFC-0102 denies a new caller-local HAC execution and no other
...
Draft RFC-0102 defines the first bounded local execution-permission policy.
...
This is specifically the Draft RFC-0102 first-policy no-alternative outcome.
It is not a general category for arbitrary policy refusal. It must not mean
runtime unavailable, runtime busy, remote receiver state, transport failure,
no static candidate, invalid request, cancellation, queueing, waiting timeout,
or remote refusal.
...
to Draft RFC-0102. The local denial is not the final request failure. If the
...
Draft RFC-0102's first policy does not apply receiver-side after
...
compatibility error containment when it reaches this policy. This RFC introduces no new
compatibility-specific public error taxonomy or envelope.
...
Draft RFC-0098 defines execution-availability semantics; Draft RFC-0099 defines
HAC's authority boundary; Draft RFC-0100 defines the first bounded
process-local scope; Draft RFC-0101 defines active execution-interval
cardinality; and Draft RFC-0102 defines the first local execution-permission
policy. This RFC defines the no-alternative failure contract required by that
policy.

This RFC cannot become Accepted before Draft RFC-0098 through Draft RFC-0102
are accepted. If any prerequisite materially changes, this RFC must be reviewed
again.
...
Rejected and deferred. Draft RFC-0102's first policy does not apply
receiver-side.
