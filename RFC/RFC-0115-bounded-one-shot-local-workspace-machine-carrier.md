...
Untrusted requests fail closed and cannot expand a grant or select a physical root. The carrier adds no network access, content/path/history persistence, or default request/response logging. The machine response is the explicit disclosure to its invoking parent. OS permissions remain authoritative. The carrier is not a sandbox and adds no same-user isolation guarantee beyond RFC-0114.
