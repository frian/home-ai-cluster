...
| Declared remote status: `status_command._collect_status` and `HttpRemoteStatusTransport` | One async client to an operator-declared HTTP or HTTPS remote | Status metadata; default client timeout and bounded per-request status timeout | Same proxy and HTTPS CA effect. |
